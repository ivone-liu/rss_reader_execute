...
class Token
{
	 ...
	 * appsercet
	 */
	public static $appsercet = 'azE8V15hAjnx';
	...
	public function token(Request $request)
	{
		...
		$user = AppidModel::where(['mobile'=>input('mobile')])->find();
		if (!empty($user)) {
			$userInfo = [
				'uid'		=>	$user['id'],
				'mobile'	=>	$user['mobile']
			...
			return self::returnMsg(500,'Mobile not exsits!');
		}
		try {
	...
	{	
		...
		$verify = AppidModel::where(['appid'=>$params['appid']])->find();
		if (empty($verify)) {
			return self::returnMsg(401,'appid 错误');
		}
		// 签名检测
		// sign = md5(appid=post.appid&mobile=post.mobile&nonce=post.nonce&timestamp=post.timestamp&key=secretkey)
		$sign = Oauth::makeSign($params, $verify['appsercet']);
		if($sign !== $params['sign']){
